"""
Export a CRP-D yard snapshot to Jin & Tanaka (2023) UCRP-IDBB input format.
...
For CRP-D the priority value written for each container is its **group ID**
(``container.priority``, which equals ``container.group`` for ZhuDup
instances).  Group IDs can repeat — the solver treats containers with the
same label as belonging to the same retrieval tier.
