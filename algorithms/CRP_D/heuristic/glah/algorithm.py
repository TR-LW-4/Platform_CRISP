...
Platform interface
------------------
Packaged for **CRP-D** (``compatible_problems``).  The 2015 paper is CRP-R
(single-bay, fixed retrieval order); CRP-D / stowage dynamics may require
further adaptation — this copy is the same port as the other GLAH packages.

Uses GlahLayout internally; outputs RelocationPlan via evaluate_plan().

NOTE:  Other GLAH packages: ``algorithms/CRP_U/heuristic/glah`` (CRP-U),
``algorithms/CRP_Time/heuristic/glah`` (CRP-Time).  Keep in sync or symlink.
"""
...
class GLAHHeuristic(BaseAlgorithm):
    ...
    name                = "Jin (2015) GLAH (CRP-D)"
    category            = "Heuristic"
    description         = (
        "[CRP-D packaging] [single-bay origin]  "
