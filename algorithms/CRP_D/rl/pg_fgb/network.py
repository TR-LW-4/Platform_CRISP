"""
Transformer-Encoder + Pointer-Decoder policy network for CRP-D.
...
Decoder query (v2)
------------------
The decoder query is composed of:
    query = target_node_embedding + global_mean_embedding
where target_node_embedding = encoder_output[:, -1, :] (the last node, which
encodes the current vessel-slot / target-group context), and global_mean_embedding
= mean over all stack nodes. This gives the decoder explicit knowledge of WHAT
to retrieve and WHERE the global yard state is, replacing the old static
start_embedding.

Input
-----
node_features : (B, N, 5)  where N = n_stacks + 1
    The last node carries the current vessel-slot context (target group).
    The first n_stacks nodes represent individual yard stacks.

    For CRP-D the 5 features are:
        [stack_id(=bay),  row(=1 fixed),  height,  has_target,  top_group]

action_masks  : (B, A)  True = forbidden action
    A = n_stacks  (pointer selects over stacks only, not vessel node).

The network normalises each feature dimension by feature_scale before encoding.
Raw integer yard-state values are fed directly from the platform observation.
"""
...
class _PointerDecoder(nn.Module):
    def __init__(self, embed_dim, num_heads, clip_constant=10.0, dropout=0.0):
        ...
        # Kept for backward-compat when loading old checkpoints; not used as query.
    ...
    def _build_query(self, encoder_output: torch.Tensor) -> torch.Tensor:
        """
        Compose the decoder query from:
          - target_node  : encoder_output[:, -1, :]   (vessel/target-group node)
          - global_mean  : mean over stack nodes encoder_output[:, :-1, :]
        Returns (B, 1, D).
        """
        target_emb = encoder_output[:, -1:, :]            # (B, 1, D)
        global_emb = encoder_output[:, :-1, :].mean(dim=1, keepdim=True)  # (B, 1, D)
        return target_emb + global_emb

    def _pointer_scores(self, query, encoder_output, action_mask):
        """
        query         : (B, 1, D)
        encoder_output: (B, N, D)  — N = n_stacks + 1 (target-group node last)
        action_mask   : (B, A)     — A = n_stacks; True = forbidden
        Returns log_softmax over A stack actions.
        """
    ...
    def forward(self, encoder_output, action_mask, greedy=False):
        query   = self._build_query(encoder_output)           # (B, 1, D)
...
class CrpdPolicyNetwork(nn.Module):
    """
    Transformer + Pointer policy network for CRP-D.
    ...
    feature_scale  : Per-feature normalisation divisors (length-5 tensor).
                     Default for CRP-D: [10, 80, 10, 1, 10]
                     (stack_id, row/slot-progress, height, has_target, group).
    """

    INPUT_DIM = 5   # [stack_id(bay), row, height, has_target, group]
    ...
    def _reshape(self, flat_obs: "torch.Tensor") -> "torch.Tensor":
        ...
        Accept flat observation from the platform:
            (B, (n_stacks + 1) * 5) or (B, n_stacks + 1, 5)
        Returns (B, n_stacks + 1, 5) normalised.
        """
        if flat_obs.dim() == 2:
    ...
    def evaluate_actions(
        ...
        Uses the same target-node + global-mean query as forward().
        """
        encoder_output = self.encode(flat_obs)
        query   = self.decoder._build_query(encoder_output)  # (B, 1, D)
