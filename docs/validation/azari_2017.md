...
**A3 — the scored plan omitted the containers that are on top at the start. Fixed in `fe2676b`.** Line numbers before the fix refer to `d043aae`. `train()` called `env.reset()` (`algorithm.py:488`), and `CRP_R.reset` retrieves every target 1, 2, … that is already on top (`problems/CRP_R.py:109-116`, `174-194`). The solver's stacks and `initial_yard` were read after that (`algorithm.py:489-495`), so the solver's targets started after those containers (`algorithm.py:77-79`) and `evaluate_plan_objectives` scored a plan without their retrievals (`algorithm.py:518-523`). GBH and CSUM retrieve a target on top themselves (`algorithm.py:327-344`, `382-395`), so the cause was in the platform layer, not in the method of the paper. On instances 1–10 of every class (f2, `plan_completeness.py`, local), 48 of 210 plans were incomplete: exactly the instances where targets are on top at the start, and the missing containers were exactly those targets (210/210). By the same rule, 199 of the 840 Caserta instances are affected (282 containers). f2 and f2vert add up per move and do not depend on the previous crane position (`core/objectives.py:261-300`), so the reported value was too low by exactly the cost of the missing retrievals (reported + missing = full plan, largest deviation 4·10⁻¹²; on the affected instances, f2 was on average 56.2 s too low); relocations were unchanged. Check 5, Check 6 and §3.6 are not affected: their scripts built the solver from the `.dat` file. Runs through `train()` (CLI, web UI) on affected instances were affected, such as data3-5-1 (§3.5).

The fix resets with `skip_auto_retrieve` (`algorithm.py:488-489`), so GBH and CSUM plan these retrievals themselves; `get_best_solution()` is unchanged, because it holds only the relocation destinations. Checked on the same 210 instances before and after the fix (`a3_fix_check.py`, `a3_compare.py`, local):
- On the 121 instances where GBH and CSUM finished their trees in both runs (36 affected), the new plan equals the missing retrievals followed by the old plan, move for move, and f2 and f2vert rise by exactly their cost; relocations are equal.
- On the 89 instances where a time limit was hit (12 affected), the same holds on 82, including all 12 affected ones. The other 7 are unaffected instances whose input did not change; their plans differ because CSUM stops on the clock (§3.6): the new f2 is lower on 4, higher on 2 and equal on 1.
- Every new plan is complete and feasible (210/210), with total movements = relocations + N.
- Replaying `get_best_solution()` on a CRP-Time environment gives the reported f2, f2vert, relocations and movements (210/210).
- data3-5-1 through the CLI and the web UI: f2 752.40, f2vert 1408.335, 6 relocations, 21 movements (screenshot `ui_azari_a3fix.png`, local).
...
- **Platform point for Wei: the web backend accepts CRP-Time only on random layouts.** `validate_source_for_problem` (`web/backend/benchmarks.py:280-301`), called when a job is started (`web/backend/api.py:230`), rejects the Caserta and Zhu sources for every problem outside `BENCH_PROBLEMS = {"CRP-R", "CRP-U"}` (`web/backend/benchmarks.py:37`, `:288-289`). A CRP-Time job on Caserta therefore fails with HTTP 422, "caserta benchmark requires CRP-R or CRP-U"; the same rule is on `origin/main`. Caserta runs of CRP-Time currently work only through the CLI (`main.py layout-run`). This blocks the standardisation requirement that each algorithm runs correctly through the web UI on the Caserta instances. **Resolved (25 Sept).** Wei asked to enable Caserta for CRP-Time and to remove random layouts for CRP-Time from the UI and the CLI. Done in `cc2c7ad`, `06e2436`, `b2af3ff` and `8de92ad`. A CRP-Time job on data3-5-1 through the web UI (f2) completes with f2 752.40, f2vert 1408.335, 6 relocations and 21 movements, identical to `main.py layout-run` (screenshot `ui_azari_a3fix.png`, local); before the A3 fix (`fe2676b`), these values missed the retrieval of container 1 (f2 710.40).
